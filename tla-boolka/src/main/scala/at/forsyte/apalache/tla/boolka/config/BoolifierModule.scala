package at.forsyte.apalache.tla.boolka.config

import at.forsyte.apalache.infra.ExceptionAdapter
import at.forsyte.apalache.infra.passes.{Pass, PassOptions, TerminalPass, WriteablePassOptions}
...
import at.forsyte.apalache.tla.boolka.passes.{BoolifierPass, BoolifierPassImpl}
...
/**
  ...
  * @author Igor Konnov
  */
class BoolifierModule extends AbstractModule {
  override def configure(): Unit = {
    ...
    bind(classOf[BoolifierPass])
      .to(classOf[BoolifierPassImpl])
    bind(classOf[Pass])
      .annotatedWith(Names.named("AfterAnalysis"))
      .to(classOf[BoolifierPass])
    // the final pass is TerminalPass
    bind(classOf[Pass])
      .annotatedWith(Names.named("AfterBoolifier"))
