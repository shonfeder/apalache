...
class ModelChecker(val checkerInput: CheckerInput,
  ...
  private def catchup(tree: HyperTree): Unit = {
  ...
  private def checkOneTransition(): Boolean = {
    ...
    val (workerState, globalActive) = sharedState.synchronized {
      (sharedState.workerStates(context.rank), context.activeNode)
    }
    workerState match {
  ...
  private def increaseDepth(): Boolean = {
    ...
    if (context.rank == 1) { // only the leader may increase the depth, the others should follow
      sharedState.synchronized {
        ...
        context.activeNode = sharedState.activeNode
      }
      ...
      // recover the context from the file, as it will smash the incremental SMT contexts into a single one
      context = WorkerContext.load(savefile, context.rank)
      context.solver.log(";;;;;;;;;;;;; RECOVERY FROM %s".format(savefile.getName))
