...
class IndexedLoopAnalyser(val checkerInput: CheckerInput,
  ...
  def checkNotLiveness(): Boolean = {
    ...
    if (loopStartIndexes.isEmpty) {
      ...
      checkFairLiveness(loopStartIndexes)
    }
  }
  ...
  private def checkFairNotLiveness(loopStartStateIndexWithAction: (Int, TlaEx)): Boolean = {
    ...
    val rewrittenState = rewriter.rewriteUntilDone(stateStack.head
                                                             ._1
                                                             .setArena(actualCellArena)
                                                             .setRex(fairNotLivenessExpression)
  ...
  private def buildTakenCondition(loopStartStateIndex: Int, action: TlaEx): TlaEx = {
    ...
    for (i <- 0 to loopStartStateIndex) {
      ...
      val sourceWithPrimedBinding = addPrimedTargetBinding(sourceState, targetState).setArena(actualCellArena)
                                                                                    .setRex(action)
