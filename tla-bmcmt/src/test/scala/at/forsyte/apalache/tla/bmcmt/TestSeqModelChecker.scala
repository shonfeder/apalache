...
import at.forsyte.apalache.tla.bmcmt.trex.{FilteredTransitionExecutor, IncrementalExecutorContext, TransitionExecutorImpl}
...
class TestSeqModelChecker extends FunSuite with BeforeAndAfter {
  ...
  test("Init + Inv => OK") {
    ...
    val ctx = new IncrementalExecutorContext(rewriter)
