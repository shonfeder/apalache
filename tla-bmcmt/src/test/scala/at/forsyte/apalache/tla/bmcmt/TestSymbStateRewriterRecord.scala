...
class TestSymbStateRewriterRecord extends RewriterBase {
  test("""RecordDomainCache: ~(dom {"a", "b"} = dom {"a", "b", "c"}) ~~> $C$k""") {
    ...
    val state = new SymbState(neq, CellTheory(), newArena2, Binding())
  ...
  test("""SE-REC-CTOR[1-2]: ["a" |-> 1, "b" |-> FALSE, "c" |-> "d"] ~~> $C$k""") {
    ...
    val state = new SymbState(record, CellTheory(), arena, Binding())
    val rewriter = create()
  ...
  test("""SE-REC-ACC[1-2]: ["a" |-> 1, "b" |-> FALSE, "c" |-> "d"]["c"] ~~> $C$k equals \"d\"""") {
    ...
    val state = new SymbState(recordAcc, CellTheory(), arena, Binding())
    val rewriter = create()
    val nextState = rewriter.rewriteUntilDone(state)
    nextState.ex match {
      case membershipEx @ NameEx(name) =>
  ...
  test("""SE-REC-CTOR[1-2] in a set: {["a" |-> 1, "b" |-> FALSE], ["a" |-> 2, "b" |-> TRUE]} ~~> $C$k""") {
    ...
    val state = new SymbState(tla.enumSet(record1, record2), CellTheory(), arena, Binding())
  ...
  test("""SE-REC-CTOR[1-2] in a set: {["a" |-> 1, "b" |-> FALSE], ["a" |-> 2, "b" |-> TRUE, "c" |-> 3]} ~~> $C$k""") {
    ...
    val state = new SymbState(tla.enumSet(tla.withType(record1, annotation), record2), CellTheory(), arena, Binding())
    val rewriter = create()
    val nextState = rewriter.rewriteUntilDone(state)
    nextState.ex match {
      case membershipEx @ NameEx(name) =>
  ...
  test("""filter-map a record (idiom): {r.c : r \in {r2 \in {["a" |-> 1], ["a" |-> 2, "c" |-> 3]}: r2.c = 3}} ~~> $C$k""") {
    ...
    val state = new SymbState(mapEx, CellTheory(), arena, Binding())
    val rewriter = create()
    rewriter.push()
    val nextState = rewriter.rewriteUntilDone(state)
  ...
  test("""SE-REC-EXCEPT:[ ["a" |-> 1, "b" |-> FALSE] EXCEPT !["a"] = 3 ]""") {
    ...
    val state = new SymbState(recExcept, CellTheory(), arena, Binding())
    val rewriter = create()
    val nextState = rewriter.rewriteUntilDone(state)
