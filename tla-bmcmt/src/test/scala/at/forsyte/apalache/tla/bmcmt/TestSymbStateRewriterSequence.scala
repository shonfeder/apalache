...
class TestSymbStateRewriterSequence extends RewriterBase {
  ...
  test("""SE-SEQ-CTOR: <<>> <: Seq(Int)""") {
    ...
    val state = new SymbState(annotatedTuple, CellTheory(), arena, Binding())
  ...
  test("""SE-SEQ-HEAD: Head(<<3, 4, 5>> <: Seq(Int))""") {
    ...
    val state = new SymbState(seqApp, CellTheory(), arena, Binding())
    val rewriter = create()
    val nextState = rewriter.rewriteUntilDone(state)
    assert(solverContext.sat())
  ...
  test("""SE-SEQ-TAIL: Tail(<<3, 4, 5>> <: Seq(Int))""") {
    ...
    val state = new SymbState(seqTail, CellTheory(), arena, Binding())
    val rewriter = create()
    val nextState = rewriter.rewriteUntilDone(state)
    assert(solverContext.sat())
    val result = nextState.asCell
  ...
  test("""SE-SEQ-SUBSEQ: SubSeq(S, 2, 4)""") {
    ...
    val state = new SymbState(subseqEx, CellTheory(), arena, Binding())
    val rewriter = create()
    val nextState = rewriter.rewriteUntilDone(state)
    assert(solverContext.sat())
    val result = nextState.asCell
  ...
  test("""SE-SEQ-APPEND: Append(S, 10)""") {
    ...
    val state = new SymbState(append, CellTheory(), arena, Binding())
    val rewriter = create()
    val nextState = rewriter.rewriteUntilDone(state)
    assert(solverContext.sat())
    val result = nextState.asCell
  ...
  test("""SE-SEQ-DOMAIN: DOMAIN SubSeq(<<3, 4, 5, 6>>, 2, 3) = {2, 3}""") {
    ...
    val state = new SymbState(domEx, CellTheory(), arena, Binding())
    val rewriter = create()
    val nextState = rewriter.rewriteUntilDone(state)
    assert(solverContext.sat())
    assertTlaExAndRestore(rewriter,
  ...
  test("""SEQ-CONCAT: <<9, 10>> \o SubSeq(S, 2, 3)""") {
    ...
    val state = new SymbState(concat, CellTheory(), arena, Binding())
    val rewriter = create()
    val nextState = rewriter.rewriteUntilDone(state)
    assert(solverContext.sat())
    val result = nextState.asCell
