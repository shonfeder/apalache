...
class TestModelChecker extends FunSuite with BeforeAndAfter {
  ...
  test("Init, OK") {
    ...
    val checkerInput = new CheckerInput(dummyModule, initTrans, nextTrans, None, None, None, None, None, None, None)
  ...
  test("determinstic Init + 2 steps (regression)") {
    ...
    val checkerInput = new CheckerInput(dummyModule, initTrans, nextTrans, None, Some(notInv), None, None, None, None, None)
    // initialize the model checker
  ...
  test("Init + Next + Inv, 10 steps, and invarianFilter") {
    ...
    val notInv = tla.not(tla.lt(tla.prime(tla.name("x")), tla.int(5)))
    val dummyModule = new TlaModule("root", List(), List())
    val checkerInput = new CheckerInput(dummyModule, initTrans, nextTrans, None, Some(notInv), None, null, null, null, null)
    // initialize the model checker
    val strategy = new BfsStrategy(checkerInput, stepsBound = 10)
  ...
  test("Init + Next + Inv, 2 steps, set assignments") {
    ...
    val dummyModule = new TlaModule("root", List(), List())
    val checkerInput = new CheckerInput(dummyModule, List(init), List(next1, next2), None, Some(notInv), None, None, None, None, None)
    // initialize the model checker
    val strategy = new BfsStrategy(checkerInput, stepsBound = 2)
  ...
  test("cInit + Init + Next, 10 steps") {
    ...
    val checkerInput = new CheckerInput(dummyModule, initTrans, nextTrans, Some(cInit), Some(notInv), None, None, None, None, None)
    // initialize the model checker
    val strategy = new BfsStrategy(checkerInput, stepsBound = 10)
    val checker = new ModelChecker(typeFinder, frexStore, hintsStore, exprGradeStore, sourceStore,
      checkerInput, strategy, Map(), debug = false, profile = false)
